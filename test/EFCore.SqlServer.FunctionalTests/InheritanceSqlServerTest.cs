    ...
    public class InheritanceSqlServerTest : InheritanceTestBase<InheritanceSqlServerFixture>
    {
        ...
        public virtual void Common_property_shares_column()
        {
            using (var context = CreateContext())
            {
                ...
                AssertSql("SugarGrams", cokeType.FindProperty("SugarGrams").Relational().ColumnName);
                AssertSql("CaffeineGrams", cokeType.FindProperty("CaffeineGrams").Relational().ColumnName);
                AssertSql("CokeCO2", cokeType.FindProperty("Carbination").Relational().ColumnName);

                AssertSql("SugarGrams", liltType.FindProperty("SugarGrams").Relational().ColumnName);
                AssertSql("LiltCO2", liltType.FindProperty("Carbination").Relational().ColumnName);

                AssertSql("CaffeineGrams", teaType.FindProperty("CaffeineGrams").Relational().ColumnName);
                AssertSql("HasMilk", teaType.FindProperty("HasMilk").Relational().ColumnName);
            }
        }

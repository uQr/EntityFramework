...
namespace Microsoft.EntityFrameworkCore.Specification.Tests
{
        ...
        where TFixture : ComplexNavigationsQueryFixtureBase<TTestStore>, new()
    {
        ...
        [ConditionalFact]
        public virtual void Explicit_GroupJoin_in_subquery_with_multiple_result_operator_take_count_doesnt_materialize_main_clause()
        {
            List<int> expected;
            using (var context = CreateContext())
            {
                expected = (from l1 in context.LevelOne.ToList()
                            where (from l1_inner in context.LevelOne.ToList()
                                   join l2 in context.LevelTwo on l1_inner.Id equals l2.Level1_Optional_Id into grouping
                                   from l2 in grouping.DefaultIfEmpty()
                                   select l1_inner).Take(20).Count() > 4
                            select l1.Id).ToList();
            }
        ...
        [ConditionalFact]
        public virtual void Explicit_GroupJoin_in_subquery_with_multiple_result_operator_take_max_doesnt_materialize()
        {
            List<int> expected;
            using (var context = CreateContext())
            {
                expected = (from l1 in context.LevelOne.ToList()
                            where (from l1_inner in context.LevelOne.ToList()
                                   join l2 in context.LevelTwo on l1_inner.Id equals l2.Level1_Optional_Id into grouping
                                   from l2 in grouping.DefaultIfEmpty()
                                   select l1_inner.Id).Take(20).Max() > 4
                            select l1.Id).ToList();
            }
